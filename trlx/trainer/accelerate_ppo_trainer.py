...
class AcceleratePPOTrainer(AccelerateRLTrainer):
    ...
    def loss(self, batch: PPORLBatch):
        ...
        print(f"PPOTrainer loss(), 'v_head.0.weight': {self.model.state_dict()['v_head.0.weight']}")
        print(f"PPOTrainer loss(), tokens.shape: {tokens.shape}")
        print(f"PPOTrainer loss(), tokens: {tokens}")
        print(f"PPOTrainer loss(), attention_mask.shape: {attention_mask.shape}")
        ...
        print(f"PPOTrainer loss(), old_logprobs.shape: {old_logprobs.shape}")
        print(f"PPOTrainer loss(), old_logprobs: {old_logprobs}")
        print(f"PPOTrainer loss(), logprobs.shape: {logprobs.shape}")
